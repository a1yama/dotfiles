...
- 採否: <指示された修正が打ち手か、対症療法か。従うなら根拠、変えるなら具体的な代替案>'
    tail_msg='より良い手段があるなら、書くだけで済ませず AskUserQuestion で採否を問うてください(使えない環境なら代替案と採否の理由を本文に書いてください)。確認して指摘と食い違ったら「違います」と根拠付きで言ってください。ただし確認できたときだけです(逆張りは鵜呑みと同じだけ害があります)。'
    ;;
  target)
...
- 採否: 従うなら根拠、変えるなら具体的な代替案'
    tail_msg='より良い手段があるなら、書くだけで済ませず AskUserQuestion で採否を問うてください(使えない環境なら代替案と採否の理由を本文に書いてください)。'
    ;;
esac
