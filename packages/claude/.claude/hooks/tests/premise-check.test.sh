...
# 完了報告は依頼ではない。手戻り語を「やったこと」として含んでも発火させない
# (回帰: 「修正してpushまで完了。」で手戻り扱いになり、報告のたびに前提節を要求していた)
run none "手戻り語を含む完了報告" '修正してpushまで完了。'
run none "実装の完了報告"         'Redis を使って実装しました'
run none "数値を含む完了報告"     'カバレッジを80%まで上げました'
run none "できました"             'ビルドできました'

# ただし問題語が同居するなら本物の手戻り
run rework "直したが動かない"     '直したけど動かないままです'
run rework "失敗の報告"           'エラーが出て失敗しました'

# 判定は最終行だけ。報告の次に依頼が続く形を落とさない
run method "報告の後に依頼が続く" '実装しました。
次は Redis を使ってキャッシュも足して'
run rework "報告の後に手戻りが続く" '実装しました。
やっぱり間違っているので直して'
